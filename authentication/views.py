...
def signup(request):
    if request.method == "POST":
        ...
        if User.objects.filter(username = username):
            messages.error(request, 'This username already exist! Try some other.')
            return redirect('home')
        
        if User.objects.filter(email = email):
            messages.error(request, 'This email address already exist! Please Try Some Other.')
            return redirect('home')
        if len(username)>10:
        ...
        #email section
        subject = "Welcome to Authenticate - Django-login!"
        message = "Hello"+ myuser.first_name + "!! \n"+ "Welcome to authentication! \n Thank you for visiting our website \n We have also sent you a confirmation email, Please confirm your email address in order to activate your account. \n\n  Thank You!"
        from_email = settings.EMAIL_HOST_USER
        to_list = [myuser.email]
        send_mail(subject, message, from_email,to_list, fail_silently=True)

        #email confirmation link section
